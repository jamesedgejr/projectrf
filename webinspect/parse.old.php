	...
	*/
	...
	foreach($session->Issues->Issue as $issue){
		...
		foreach($issue->ReportSection as $reportsection){
			...
			if($row_lookup == ""){
				$sql = "INSERT INTO webinspect_xml (agency, application, url, host, port, vulnID, name, severity, summary, execution, implication, fix, referenceinfo, rawrequest, request_method, request_path, request_fullquery, request_fullpostdata, request_cookie) VALUES ('$agency', '$application', '$url', '$host', '$port', '$vulnID', '$name', '$severity', '$summary', '$execution', '$implication', '$fix', '$referenceinfo', '$raw_request', '$request_method', '$request_path', '$request_fullquery', '$request_fullpostdata', '$request_cookie')";
