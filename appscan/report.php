...
$myDir = "/var/www/projectRF/appscan/csvfiles/";
...
$sql = 	"SELECT
			...
			AppScan_Issues.TestHttpTraffic,
...
while($row = $results->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$url = $row["Url"];
	$difference = htmlentities($row["Difference"]);
	$difference_temp1 = split('->',$row["Difference"]);
	$difference_temp2 = split('=', $difference_temp1[1]);
	$parameter = $difference_temp2[0];
	$original_http_traffic = htmlentities($row["OriginalHttpTraffic"]);
	$test_http_traffic = htmlentities($row["TestHttpTraffic"]);
	$validation_string = $row["Validation_String"];
	$reasoning = $row["Reasoning"];
	
	
	$origHttpTrafficArray = split("[\n][\n]", $original_http_traffic);
	
	echo "<hr><pre><code>$origHttpTrafficArray[0]</code></pre><hr><code>$origHttpTrafficArray[1]</code><hr><code>$origHttpTrafficArray[2]</code><hr>";
	
	/*
	echo "($severity) Threat:  $threatClassification_name [$advisory_name]<br>";
	echo "<pre><code><HR>PARAMETERS<HR>$difference</code></pre><br>";
	echo "<code><HR>VALIDATION STRING<HR>$validation_string</code><br>";
	echo "<pre><code><hr>ORIGINAL HTTP TRAFFIC<hr>$original_http_traffic</code></pre><br>";
	echo "<pre><hr>TEST HTTP TRAFFIC<hr>$test_http_traffic</pre><br>";
	echo "<pre><code></code></pre><br>";
	*/
}
?>
<html>
