...
$parseID = rand(100000000000000,999999999999999);
...
foreach($xml->nodes->node as $node){
	...
	$sql = "INSERT INTO nexpose_nodes
			...
			";
	$stmt = $db->prepare($sql);
	$node_device_id = $parseID . $node['device-id'];
	$sql_data = array(
						$node['address'],
						$node['status'],
	...
	<names>
<name>VILLA</name>
<name>asvlab.org</name>
</names>
<fingerprints>
<os  certainty="0.85" device-class="General" vendor="Ubuntu" family="Linux" product="Linux" version="10.04"/>
<os  certainty="0.70" device-class="General" vendor="Linux" family="Linux" product="Linux" version="2.6.32"/>
<os  certainty="0.60" product="UNIX"/>
...
*/
	...
	foreach($node->fingerprints->os as $os){
		...
		$sql = "INSERT INTO nexpose_device_fingerprints
			...
			";
		$stmt = $db->prepare($sql);
		$fingerprint_node_id = $parseID . $node['device-id'];
		$sql_data = array(	
							$fingerprint_node_id,
							$os['certainty'],
							$os['device-class'],
	...
	/*
	<tests>
	<test id="udp-ipid-zero" key="" status="vulnerable-version" scan-id="17" vulnerable-since="20150325T140059607" pci-compliance-status="pass">

	<Paragraph>
	<Paragraph>Received UDP packet with IP ID of zero:
		<UnorderedList>
			<ListItem>
				<Paragraph preformat="true">IPv4  SRC[74.82.129.7]  TGT[74.63.177.250]
                     .....
	...
	*/
	foreach ($node->tests->test as $test){
		$sql = "INSERT INTO nexpose_tests
			...
			";
		$stmt = $db->prepare($sql);
		$test_scan_id = $parseID . $test['scan-id'];
		$test_node_device_id = $parseID . $node['device-id'];
		$sql_data = array(	
							$test['id'],
							$test_scan_id,
							$test_node_device_id,
...
<service name="FTP">
<fingerprints>
<fingerprint  certainty="0.90" family="ProFTPD" product="ProFTPD" version="1.3.1"/>
</fingerprints>
<configuration>
<config name="ftp.banner">220 ProFTPD 1.3.1 Server (ASVlab FTP) [74.82.129.7]</config>
<config name="ftp.plaintext.authentication">true</config>
<config name="ftp.supports-starttls">false</config>
</configuration>
<tests>
<test id="ftp-proftpd-command-buffer-size-dos" key="" status="vulnerable-version" scan-id="17" vulnerable-since="20150325T140059607" pci-compliance-status="fail">
...
*/	
	...
	foreach($node->endpoints->endpoint as $endpoint){
		$sql = "INSERT INTO nexpose_endpoints
			...
			";
		$stmt = $db->prepare($sql);
		$endpoint_node_device_id = $parseID . $node['device-id'];
		$sql_data = array(	
							$endpoint_node_device_id,
							$endpoint['protocol'],
							$endpoint['port'],
							$endpoint['status'],
		...
		foreach($endpoint->services->service->tests->test as $service_test){
			$sql = "INSERT INTO nexpose_tests
				...
				";
			$stmt = $db->prepare($sql);
			$service_test_scan_id = $parseID . $service_test['scan-id'];
			$service_test_device_id = $parseID . $node['device-id'];
			$sql_data = array(	
								$service_test['id'],
								$service_test_scan_id,
								$service_test_device_id,
								$endpoint_id,
								$service_test['key'],
								$service_test['status'],
