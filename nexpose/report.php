...
$scan_startTime = $_POST["scan_startTime"];
$scan_endTime = $_POST["scan_endTime"];
$scan_id = $_POST["scan_id"];


$isVulnTitle = $_POST["isVulnTitle"];
$isTag = $_POST["isTag"];
$isVulnInfo = $_POST["isVulnInfo"]; 
$isVulnOut = $_POST["isVulnOut"];

$isService = $_POST["isService"];
$isCvss = $_POST["isCvss"];
$isVulnPub = $_POST["isVulnPub"];
$isExploit = $_POST["isExploit"];	

$isDescription = $_POST["isDescription"];
$isSolution = $_POST["isSolution"];

$isApple = $_POST["isApple"];
$isBid = $_POST["isBid"];
$isCve = $_POST["isCve"];
$isCert = $_POST["isCert"];
$isMS = $_POST["isMS"];
$isOsvdb = $_POST["isOsvdb"];
$isRedHat = $_POST["isRedHat"];
$isURL = $_POST["isURL"];
$isXF = $_POST["isXF"];



$isAffected = $_POST["isAffected"];
...
<head>
 ...
 http-equiv="content-type">
  <title><?php echo "$agency - Nexpose Vulnerability Report - $scan_name";?></title>
  <!-- I know this is a big old mess with the jQuery inserted below but it allows you to save the report as one HTML file -->
...
<table width="850px" class="main">
	<tr><td class="left">Nexpose Vulnerability Tag Categories</td></tr>
	<tr><td class="right">
	<?php  
		$printTags = "";
		foreach($tags as $t){ 
		...
		$printTags = substr($printFamily,0,-2);
		echo "$printTags";	
	?>
	</td></tr>
</table>
<table width="850px" class="main">
	...
	<?php  
		...
		foreach($nodeArray as $nA){ 
			...
			echo "<td class=right><p>" . $temp_nA[0] . "</p></td>";
...
Inner Join nexpose_tests ON nexpose_vulnerabilities.vuln_id = nexpose_tests.test_id
Inner Join nexpose_scans ON nexpose_scans.scan_id = nexpose_tests.scan_id
Inner Join nexpose_tmp_nodes ON nexpose_tmp_nodes.node_device_id = nexpose_tests.device_id
WHERE
...
$where_data = array($agency, $filename, $scan_name, $scan_startTime, $scan_endTime, $scan_id);
  ...
  <tr>
	<td class="top"><p>
		<?php 
			if($isVulnTitle == "yes"){
					if($isTag == "yes"){ 
						$tag_sql = "SELECT DISTINCT
										nexpose_tags.tag
									FROM
										nexpose_tags
									WHERE
										nexpose_tags.vuln_id =  ?
								";
						$tag_stmt = $db->prepare($tag_sql);
						$tag_stmt->execute(array($row['vuln_id']));
						$tags = "";
...
} elseif ($row['cvssScore'] < 10 && $row['cvssScore'] >= 7){
	$risk_factor = "High";
} elseif ($row['cvssScore'] < 7 && $row['cvssScore'] >= 4){
	$risk_factor = "Medium";
} elseif($row['cvssScore'] < 4 && $row['cvssScore'] >= 0) {
	$risk_factor = "Low";
}
...
?>
 <tr>
	...
	<td class="right" align="center" width="16%"><p><?php echo $row["cvssScore"]; ?></p></td>
	<td class="top" align="center" width="16%"><p>Vector</p></td>
	<td class="right" align="center" width="20%"><p><?php echo $row["cvssVector"]; ?></p></td>
  </tr>
<?php }	?>
</table>
...
<?php if($isDescription == "yes"){ 

$description = htmlspecialchars_decode($row["description"], ENT_QUOTES);
$description = str_replace('<Paragraph preformat="true">',"<pre>", $description);
$description = str_replace("<Paragraph>","<pre>", $description);
$description = str_replace("</Paragraph>","</pre>", $description);
$description = str_replace("<ListItem>","<li>", $description);
$description = str_replace("</ListItem>","</li>", $description);
$description = str_replace("<UnorderedList>","<ul>", $description);
$description = str_replace("</UnorderedList>","</ul>", $description);
$description = str_replace("<URLLink ","<a ", $description);
$description = str_replace("</URLLink>","</a>", $description);
$description = str_replace("LinkURL=","href=", $description);
$description = str_replace("LinkTitle=","title=", $description);
$description = str_replace('"/>','">HERE</a>', $description);
...
<?php if($isSolution == "yes"){ 

$solution = htmlspecialchars_decode($row["solution"], ENT_QUOTES);
$solution = str_replace('<Paragraph preformat="true">',"<pre>", $solution);
$solution = str_replace("<Paragraph>","<pre>", $solution);
$solution = str_replace("</Paragraph>","</pre>", $solution);
$solution = str_replace("<ListItem>","<li>", $solution);
$solution = str_replace("</ListItem>","</li>", $solution);
$solution = str_replace("<UnorderedList>","<ul>", $solution);
$solution = str_replace("</UnorderedList>","</ul>", $solution);
$solution = str_replace("<URLLink ","<a ", $solution);
$solution = str_replace("</URLLink>","</a>", $solution);
$solution = str_replace("LinkURL=","href=", $solution);
$solution = str_replace("LinkTitle=","title=", $solution);
$solution = str_replace('"/>','">HERE</a>', $solution);
...
<table width="100%" class="main">
<?php if($isExploit == "yes"){ ?>
  <tr>
	<td class="left" width="15%"><p>Exploitability:</p></td>
	<td class="right">
	
	<table width="100%">
	  <tr>
		<td class="left" width="25%"><p>Vulnerability Publication Date</p></td>
		<td class="right" width="25%"><p><?php echo "$vuln_publication_date"; ?></p></td>
		<td class="left width="25%""><p>Patch Publication Date</p></td>
		<td class="right" width="25%"><p><?php echo "$patch_publication_date"; ?></p></td>
	  </tr>
	  <tr>
		<td class="left"><p>Exploitability Ease</p></td>
		<td class="right"><p><?php echo "$exploitability_ease"; ?></p></td>
		<td class="left"><p>Exploit Framework(s)</p></td>
		<td class="right"><p>
			<?php 
				if($exploit_framework_metasploit == "true"){ echo " 'Rapid7 Metasploit' ";}
				if($exploit_framework_canvas == "true"){ echo " 'Immunity Canvas' ";}
				if($exploit_framework_core == "true"){ echo " 'Core Impact' ";}
				if($exploit_framework_d2_elliot == "true"){ echo " 'D2 Elliot' ";}
			?>
		</p></td>
	  </tr>
	  <?php if($metasploit_name != ""){ echo "<tr><td colspan=\"2\" class=\"left\"><p align=center>Metasploit Module Name</p></td><td colspan=\"2\" class=\"right\"><p align=center>" . $metasploit_name . "</p></td></tr>";} ?>
	  <?php if($canvas_package != ""){ echo "<tr><td colspan=\"2\" class=\"left\"><p align=center>Immunity Canvas Package Name</p></td><td colspan=\"2\" class=\"right\"><p align=center>" . $canvas_package . "</p></td></tr>";} ?>
	  <?php if($d2_elliot_name != ""){ echo "<tr><td colspan=\"2\" class=\"left\"><p align=center>D2 Elliot Name</p></td><td colspan=\"2\" class=\"right\"><p align=center>" . $d2_elliot_name . "</p></td></tr>";} ?>
	</table>
...
?>
  <tr>
	...
	<td class="right">
	<?php 
		if($listCount>5){ echo "<div class=\"text-container\"><div class=\"text-content short-text\">";}
		foreach ($bidList as $bid){
			if($bid != ""){
				echo "<a href=\"http://www.securityfocus.com/bid/$bid\" target=\"_blank\">$bid</a> (http://www.securityfocus.com/bid/$bid)<br>";
			}//end if
		}
		if($listCount>5){echo "</div><div class=\"show-more\"><a href=\"#\">Show more</a></div></div>";}
	?>
	</td>
  </tr>
<?php } ?>
  ...
  <tr>
	...
	<?php
		...
		foreach ($certList as $cert){
			if($cert != ""){
			echo "<a href=\"http://www.cert.org/advisories/$cert.html\" target=\"_blank\">$cert</a> (http://www.cert.org/advisories/$cert.html)<br>";
			}//end if
		}
		if($listCount>5){echo "</div><div class=\"show-more\"><a href=\"#\">Show more</a></div></div>";}
	?>
  ...
  <tr>
	...
	<?php
		...
		foreach ($cveList as $cve){
			if($cve != ""){
				echo "<a href=\"http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=$cve\" target=\"_blank\">$cve</a> (http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=$cve)<br>";
			}//end if
		}
		if($listCount>5){echo "</div><div class=\"show-more\"><a href=\"#\">Show more</a></div></div>";}
	?>
...
?>
  <tr>
	...
	<?php
		if($listCount>5){ echo "<div class=\"text-container\"><div class=\"text-content short-text\">";}
		foreach ($msftList as $msft){
			if($msft != ""){
			echo "<a href=\"http://www.microsoft.com/technet/security/bulletin/$msft.mspx\" target=\"_blank\">$msft</a> (http://www.microsoft.com/technet/security/bulletin/$msft.mspx)<br>";
			}//end if
		}
		if($listCount>5){echo "</div><div class=\"show-more\"><a href=\"#\">Show more</a></div></div>";}
	?>
...
?>
  <tr>
	...
	<?php
		if($listCount>5){ echo "<div class=\"text-container\"><div class=\"text-content short-text\">";}
		foreach ($osvdbList as $osvdb){
			if($osvdb != ""){
			echo "<a href=\"http://osvdb.org/show/osvdb/$osvdb\" target=\"_blank\">$osvdb</a> (http://osvdb.org/show/osvdb/$osvdb)<br>";
			}//end if
		}
		if($listCount>5){echo "</div><div class=\"show-more\"><a href=\"#\">Show more</a></div></div>";}
	?>
...
<?php 
$redhatList = explode(",", $row["redhatList"]);
$listCount = count($redhatList);
if($isRedHat == "yes" && $listCount > 1) { 
#Information_Assurance_Vulnerability_Alert
?>
  <tr>
	<td class="left"><p>RedHat:</p></td>
	<td class="right">
	<?php
		if($listCount>5){ echo "<div class=\"text-container\"><div class=\"text-content short-text\">";}
		foreach ($redhatList as $redhat){
			if($redhat != ""){
				echo "<a href=\"https://rhn.redhat.com/errata/$redhat.html\" target=\"_blank\">$redhat</a> (https://rhn.redhat.com/errata/$redhat.html)<br>";
...
$listCount = count($urlList);
if($isURL == "yes" && $listCount > 1) { 
?>
  <tr>
	<td class="left"><p>URLs:</p></td>
	<td class="right">
	<?php
		if($listCount>5){ echo "<div class=\"text-container\"><div class=\"text-content short-text\">";}
...
<?php 
$xfList = explode(",", $row["xfList"]);
$listCount = count($xfList);
if($isXF == "yes" && $listCount > 1) {
?>
  <tr>
	<td class="left"><p>x-Force:</p></td>
	<td class="right">
	<?php
		if($listCount>5){ echo "<div class=\"text-container\"><div class=\"text-content short-text\">";}
		foreach ($xfList as $xf){
			if($xf != ""){
			echo "<a href=\"https://exchange.xforce.ibmcloud.com/vulnerabilities/$xf\" target=\"_blank\">$xf</a> (https://exchange.xforce.ibmcloud.com/vulnerabilities/$xf)<br>";
...
$data = array($row["vuln_id"],$agency, $filename, $scan_name, $scan_startTime, $scan_endTime, $scan_id);
  ...
  <tr>
	...
	<td class="right">
		...
		while($host_row = $host_stmt->fetch(PDO::FETCH_ASSOC)) {
			$test_paragraph = htmlspecialchars_decode($host_row["test_paragraph"], ENT_QUOTES);
			$test_paragraph = str_replace('<Paragraph preformat="true">',"", $test_paragraph);
			$test_paragraph = str_replace("<Paragraph>","", $test_paragraph);
			$test_paragraph = str_replace("</Paragraph>","", $test_paragraph);
			$test_paragraph = str_replace("<ListItem>","", $test_paragraph);
			$test_paragraph = str_replace("</ListItem>","", $test_paragraph);
			$test_paragraph = str_replace("<UnorderedList>","", $test_paragraph);
			$test_paragraph = str_replace("</UnorderedList>","", $test_paragraph);
		...
		$os_sql = "
				...
				FROM
					...
					Inner Join nexpose_tests ON nexpose_scans.scan_id = nexpose_tests.scan_id
					Inner Join nexpose_nodes ON nexpose_tests.device_id = nexpose_nodes.node_device_id
					Inner Join nexpose_device_fingerprints ON nexpose_nodes.node_device_id = nexpose_device_fingerprints.device_id
				WHERE
					nexpose_device_fingerprints.device_certainty >=  '0.8' AND
					nexpose_nodes.node_address = ? AND
				...
				LIMIT 1
		";
		$os_data = array($host_row["node_address"],$agency, $scan_name, $scan_startTime, $scan_endTime, $scan_id);
		$os_stmt = $db->prepare($os_sql);
		$os_stmt->execute($os_data);
		$os_row = $os_stmt->fetch(PDO::FETCH_ASSOC);
		//get endpoint/service details
		?>
		<tr>
		  <td class="right"><p><?php echo $host_row["node_address"]?></p></td>
		  <td class="right"><p><?php if($host_row["node_hardware_address"] != "000000000000"){echo $host_row["node_hardware_address"];}?></p></td>
		  <td class="right"><p><?php echo $host_row["node_name"];?></p></td>
		  <td class="right"><p><?php echo $host_row["node_name"];?></p></td>
		  <td class="right"><p><?php echo $os_row["device_vendor"] . " " . $os_row["device_family"] . " " . $os_row["device_version"];?></p></td>
		  <?php if($isService == "yes"){ ?>
		  <td class="right"><p><?php echo $host_row["service_name"]."(".$host_row["endpoint_port"].")";?></p></td>
		  <td class="right"><p><?php echo $host_row["endpoint_protocol"];?></p></td>
		  <?php } ?>
		</tr>
		<?php if($isVulnOut == "yes" && $host_row["test_paragraph"] != ""){ 
			echo "<tr><td class=\"right\" colspan=\"";
				$number = (isService == "yes") ? 5:7;
				echo "$number";
			echo "\"><pre>".$test_paragraph."</pre></td></tr>";
		} ?>
		<?php
		}//end while
		?>
...
<?php if($isVulnInfo == "yes"){ ?>
<table width="100%" class="main">
  <tr>
	<td class="top" width="14%" align="right"><p>Vulnerability ID</p></td>
	<td class="right" width="20%" align="center"><p><?php echo $row["vuln_id"]; ?></p></td>
	<td class="top" width="13%" align="right"><p>Publish Date</p></td>
	<td class="right" width="20%" align="center"><p><?php echo $row["vuln_published"]; ?></p></td>
	<td class="top" width="13%" align="right"><p>Revision Date</p></td>
	<td class="right" width="20%" align="center"><p><?php echo $row["vuln_modified"]; ?></p></td>
  </tr>	
</table>
<?php } ?>
...
<textarea style="width:100%; height: 10%" id="<?php echo $pluginID?>"></textarea>
<div style="clear:both;"></div>
</div>
<div id="<?php echo $pluginID . "Result" ?>"></div>
<script>
$('#<?php echo $pluginID?>').bind('input propertychange', function() {

   $("#<?php echo $pluginID . "Result" ?>").html($(this).val().replace(/\n/g,'<br/>'));
