...
if($vuln_cat == "isCustom"){
	$pluginID_array = explode(",", $_POST["custom"]);
}
...
if($vuln_cat == "isSSLIssues"){
	...
	array_push($pluginID_array, "26928","20007","57582","65821","42873","60108","69551","42880","77200","73412","78479","80035","35291","84152","66848","83875","83738");
...
if($vuln_cat == "isSSHIssues"){
	...
	array_push($pluginID_array, "70658","71049","44076","44075","19592","44077","44078","44079","44065","44073","31737","44080","10882","80556","81606","53841");
}
...
if($vuln_cat == "isHPIssues"){
	...
	array_push($pluginID_array, "34694","38832","46015","46677","49272","53532","58811","59851","66541","69020","76345","72959","10746","70118","73639");
}
if($vuln_cat == "isMSIssues"){
	...
	array_push($pluginID_array, "11808","11835","12054","12209","13852","15572","18502","19408","21193","21655","22034","34311","34477","35362","35635","40887","48405","53503","58435","79638");
}
...
if($vuln_cat == "isCleartext"){
	...
	fwrite($fh_exposure, "\"Exposure Rating/ Mitigation Weight\",\"MEDIUM\",\"6\"\n");
	array_push($pluginID_array, "10079","34324","42263","19782");
}
...
$nessus_lookup_port_sql .="
								WHERE
									...
									nessus_results.port <> 3389 AND
									nessus_results.severity <> 0
								";	//if you want to not include a specific port in the results...say exclude Terminal Services from the SSL report...then nessus_results.port <> 3389
$data = array($agency);
...
while($nessus_port_row = $nessus_lookup_port_stmt->fetch(PDO::FETCH_ASSOC)){
	...
	$ip_addr = $nessus_port_row["ip_addr"];
	$verify_array["$ip_addr"."."."$port"] = 1;
}

//get all IP addresses from NMAP table, for all OPEN ports, for the agency
$data = array($agency);
$nmap_sql = "SELECT DISTINCT
				nmap_hosts_xml.address_addr
			FROM
				nmap_runstats_xml
			LEFT JOIN nmap_hosts_xml ON nmap_hosts_xml.runstats_id = nmap_runstats_xml.id
			...
			";
$nmap_stmt = $db->prepare($nmap_sql);
$nmap_stmt->execute($data);
$nmap_host_array = $nmap_stmt->fetchAll(PDO::FETCH_COLUMN);

//get all IP addresses from NESSUS table for agency
$nessus_sql = 	"SELECT DISTINCT
					nessus_tags.ip_addr
				FROM
					nessus_tags
				INNER JOIN nessus_results ON nessus_results.tagID = nessus_tags.tagID
...
$nessus_stmt->execute($data);
$nessus_host_array = $nessus_stmt->fetchAll(PDO::FETCH_COLUMN);


$nmap_only_array = array_diff($nmap_host_array,$nessus_host_array);
$nessus_only_array = array_diff($nessus_host_array,$nmap_host_array);
$hosts_array = array_unique(array_merge($nmap_host_array,$nessus_host_array));
sort($hosts_array);

//print_r($nmap_only_array);
//print_r($nessus_only_array);
//print_r($hosts_array);
fwrite($fh_vuln, "\"IP\",\"Port\",\"Observations\"\n");
foreach($hosts_array as $hA){
	...
	$nmap_lookup_sql = "SELECT DISTINCT
						...
						WHERE
							...
							nmap_ports_xml.port_state = 'open'
						";
	$nmap_lookup_data = array($agency,$hA);
	$nnmap_lookup_stmt = $db->prepare($nmap_lookup_sql);
	$nnmap_lookup_stmt->execute($nmap_lookup_data);

	//First column of the CSV file is the IP Address
	fwrite($fh_vuln, "\"$hA\",\"");
	//
	while($nmap_row = $nnmap_lookup_stmt->fetch(PDO::FETCH_ASSOC)){

		$port_protocol = strtoupper($nmap_row["port_protocol"]);
		$port_portid = $nmap_row["port_portid"];
		$port_service_name = $nmap_row["port_service_name"];
		
		//if the Nmap port is in the array we created of all vulnerable IPaddress:Port pairs
		if($verify_array["$hA"."."."$port_portid"] == 1){
	...
	while($nmap_row = $nmap_lookup_stmt->fetch(PDO::FETCH_ASSOC)){

		$port_protocol = strtoupper($nmap_row["port_protocol"]);
		$port_portid = $nmap_row["port_portid"];
		$port_service_name = $nmap_row["port_service_name"];
		$port_service_product = $nmap_row["port_service_product"];
		$port_service_version = $nmap_row["port_service_version"];
		$port_service_extrainfo = $nmap_row["port_service_extrainfo"];
		if($verify_array["$hA"."."."$port_portid"] == 1){
			array_push($serviceList_array, $port_service_name);
			array_push($servicePort_array, $port_portid);
			if($port_service_name != "msrpc"){
	...
	while($nessus_row = $nessus_lookup_pluginName_stmt->fetch(PDO::FETCH_ASSOC)){
		...
		$risk_factor = $nessus_row["risk_factor"];
		fwrite($fh_vuln, "($risk_factor) $pluginName\n");
	}
	fwrite($fh_vuln, "\"\n");


}//end foreach host_array
