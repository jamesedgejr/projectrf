...
while($nessus_port_row = $nessus_lookup_port_stmt->fetch(PDO::FETCH_ASSOC)){
	...
	$protocol = strtoupper($nessus_port_row["protocol"]);
	...
	$nmap_lookup_sql = "SELECT DISTINCT
						...
						";
	$nmap_lookup_data = array($agency,$ip_addr,$port);
	...
	if($num_rows > 0){
		foreach($nmap_lookup_Array as $nlA){
			$port_protocol = strtoupper($nlA["port_protocol"]);
			$port_portid = $nlA["port_portid"];
			$port_service_name = $nlA["port_service_name"];
	...
	if($num_rows > 0){
		foreach($nmap_lookup_Array as $nlA){
			...
			$port_portid = $nlA["port_portid"];
			$port_service_name = $nlA["port_service_name"];
			$port_service_product = $nlA["port_service_product"];
			$port_service_version = $nlA["port_service_version"];
			$port_service_extrainfo = $nlA["port_service_extrainfo"];
			if($port_service_name != "msrpc"){
	...
	$nessus_lookup_pluginName_sql .="
									...
									";	
	$data = array($agency,$ip_addr,$severity);
